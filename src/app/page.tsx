...
const FAQ = [
  ...
  ["Where does my draft go?", "Your essay and session history are stored in your account. The draft is sent to the active feedback model only when you request a read. The current provider policy is shown before you paste and explained on the Privacy page."],
];
...
export default async function Home() {
  ...
  return (
    <main className="landing-shell min-h-screen overflow-hidden">
      ...
      <section className="mx-auto max-w-7xl px-5 pb-20 pt-12 sm:px-8 sm:pt-20 lg:px-10 lg:pb-28">
        <div className="hero-grid items-center gap-12 lg:gap-10">
          <Reveal className="max-w-2xl">
            ...
            <div className="mt-9 flex flex-wrap items-center gap-4"><Link href={signedIn ? appHref : "#try"} className="inline-flex items-center gap-2 rounded-full bg-accent px-6 py-3.5 text-sm font-semibold text-white shadow-[0_14px_30px_-15px_rgba(91,92,226,0.7)] transition hover:-translate-y-0.5 hover:bg-[#4849c8]">{signedIn ? "Continue writing" : "Try a sample read"}<Arrow /></Link><a href="#method" className="inline-flex items-center gap-2 rounded-full border border-line px-5 py-3.5 text-sm font-medium text-ink transition hover:border-ink/30 hover:bg-white">See how it works</a></div>
      ...
      <section id="try" className="scroll-mt-8 mx-auto max-w-7xl px-5 py-24 sm:px-8 lg:px-10 lg:py-32">
        <Reveal>
          <div className="mb-10 max-w-2xl">
            <p className="section-eyebrow">Try before you sign in</p>
            <h2 className="mt-5 font-display text-4xl font-medium leading-[1.02] tracking-[-0.05em] text-ink sm:text-6xl">Feel the difference in one question.</h2>
            <p className="mt-5 max-w-xl leading-7 text-muted">This sample is synthetic. You can test the complete move from draft to question to usable material without creating an account or sending us your writing.</p>
          </div>
          <SampleReadDemo />
        </Reveal>
      </section>

      <section id="method" className="mx-auto max-w-7xl px-5 pb-24 sm:px-8 lg:px-10 lg:pb-32">
      ...
      <section className="mx-auto max-w-7xl px-5 pb-24 sm:px-8 lg:px-10 lg:pb-32"><Reveal><div className="privacy-band rounded-2xl border border-line px-6 py-8 sm:flex sm:items-center sm:justify-between sm:gap-10 sm:px-9"><div className="flex gap-4"><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-accent-soft text-accent"><LogoMark className="h-6 w-6" /></div><div><p className="font-medium text-ink">Know exactly where your draft goes.</p><p className="mt-1 max-w-2xl text-sm leading-6 text-muted">Drafts stay in your account and leave it only when you request feedback. {policy.safeForPersonalContent ? "The active paid model does not use submitted content to improve its products." : "This deployment uses Gemini's unpaid tier, whose terms permit Google to use submitted content to improve products and allow human review. Do not paste sensitive information."}</p></div></div><Link href="/privacy" className="mt-5 inline-flex shrink-0 items-center gap-2 text-sm font-medium text-accent sm:mt-0">Privacy, explained <Arrow /></Link></div></Reveal></section>
      ...
      <section className="mx-auto max-w-7xl px-5 pb-16 sm:px-8 lg:px-10"><Reveal><div className="final-cta rounded-[2rem] px-7 py-12 text-center sm:px-12 sm:py-16"><p className="section-eyebrow text-accent">Start with the story you already have</p><h2 className="mx-auto mt-5 max-w-2xl font-display text-4xl font-medium leading-[1.02] tracking-[-0.05em] text-ink sm:text-6xl">Your essay does not need another author.</h2><p className="mx-auto mt-5 max-w-lg leading-7 text-muted">It needs the right question, asked at the right line.</p><Link href={signedIn ? appHref : "#try"} className="mt-8 inline-flex items-center gap-2 rounded-full bg-ink px-6 py-3.5 text-sm font-semibold text-white transition hover:-translate-y-0.5 hover:bg-accent">{signedIn ? "Open my workspace" : "Try the sample"}<Arrow /></Link></div></Reveal></section>
